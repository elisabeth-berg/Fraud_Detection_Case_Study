...
def drop_unwanted(data):
    ...
    wanted_columns = ['body_length', 'channels',
                      ...
                      'org_cluster_one', 'org_cluster_two', 'org_cluster_three',
                      'org_cluster_four', 'org_cluster_five',
...
def categorize_descriptions(org_and_event_desc):
    """
    ...
    """
    cluster_org_and_descr_df = org_and_event_desc.copy()
    new_df = org_and_event_desc[['description','org_desc']]
    descr_means, org_means, descr_tfidf, org_tfidf = load_k_means_model_objects()
    new_df_new = update_data_frame(new_df)
    desc_counts_tfidf, org_counts_tfidf = vecrtorize_new_data(new_df_new, descr_tfidf, org_tfidf)
    desc_matrix = desc_counts_tfidf.todense()
    org_matrix = org_counts_tfidf.todense()
    descr_centers, org_centers = cluster_centers()
    dist_one_org = np.apply_along_axis(distance_to_cluster_one_org, 1, org_matrix)
    dist_two_org = np.apply_along_axis(distance_to_cluster_two_org, 1, org_matrix)
    dist_three_org = np.apply_along_axis(distance_to_cluster_three_org, 1, org_matrix)
    dist_four_org = np.apply_along_axis(distance_to_cluster_four_org, 1, org_matrix)
    dist_five_org = np.apply_along_axis(distance_to_cluster_five_org, 1, org_matrix)
    ...
    cluster_org_and_descr_df["org_cluster_one"] = dist_one_org
    cluster_org_and_descr_df["org_cluster_two"] = dist_two_org
    cluster_org_and_descr_df["org_cluster_three"] = dist_three_org
    cluster_org_and_descr_df["org_cluster_four"] = dist_four_org
    cluster_org_and_descr_df["org_cluster_five"] = dist_five_org

    cluster_org_and_descr_df['cluster_one'] = dist_one
    cluster_org_and_descr_df['cluster_two'] = dist_two
    cluster_org_and_descr_df['cluster_three'] = dist_three
    cluster_org_and_descr_df['cluster_four'] = dist_four
    cluster_org_and_descr_df['cluster_five'] = dist_five
    return cluster_org_and_descr_df
...
def vectorize_text_descriptons(new_df):
    ...
    words_counts_tfidf_org = vect_tfidf.fit_transform(new_df['org_desc_no_HTML'])
    return words_counts_tfidf, words_counts_tfidf_org
...
def load_k_means_model_objects():
    ...
    org_means = pickle.load(open("../kmean_org_desc_models.pkl", "rb"))
    desc_tfidf = pickle.load(open("../desc_vectorizer_final.pkl", "rb"))
    org_tfidf = pickle.load(open("../org_vectorizer_final.pkl", "rb"))
    return descr_means, org_means, desc_tfidf, org_tfidf


def vecrtorize_new_data(data, descr_tfidf, org_tfidf):
    """return tfidf transformed to new data"""
    descr_counts_tfidf = descr_tfidf.transform(data['description_no_HTML'])
    org_counts_tfidf = org_tfidf.transform(data['org_desc_no_HTML'])
    return descr_counts_tfidf, org_counts_tfidf


def cluster_centers():
    """return cluster centers for descriptions and org_discriptions"""
    descr_means = pickle.load(open("../kmeans_desc_models.pkl", "rb"))
    org_means = pickle.load(open("../kmean_org_desc_models.pkl", "rb"))
    descr_centers = descr_means.cluster_centers_
    org_centers = org_means.cluster_centers_
    return descr_centers, org_centers
...
def distance_to_cluster_one_org(row):
    descr_centers, org_centers = cluster_centers()
    cluster_center = org_centers[0]
    return distance_to_cluster(row, cluster_center)


def distance_to_cluster_two_org(row):
    descr_centers, org_centers = cluster_centers()
    cluster_center = org_centers[1]
    return distance_to_cluster(row, cluster_center)


def distance_to_cluster_three_org(row):
    descr_centers, org_centers = cluster_centers()
    cluster_center = org_centers[2]
    return distance_to_cluster(row, cluster_center)


def distance_to_cluster_four_org(row):
    descr_centers, org_centers = cluster_centers()
    cluster_center = org_centers[3]
    return distance_to_cluster(row, cluster_center)


def distance_to_cluster_five_org(row):
    descr_centers, org_centers = cluster_centers()
    cluster_center = org_centers[4]
    return distance_to_cluster(row, cluster_center)
